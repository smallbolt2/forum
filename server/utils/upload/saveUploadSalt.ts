...
export const saveUploadSalt = async (
  ...
  await useStorage().setItem(`toolset:resource:${salt}`, storedValue)
}

export const getUploadCache = async (salt?: string) => {
  const res = await useStorage().getItem(`toolset:resource:${salt}`)
  if (!res) {
    return
  }
...
export const removeUploadCache = async (salt: string) => {
  await useStorage().removeItem(`toolset:resource:${salt}`)
}
