...
export const sendVerificationCodeEmail = async (
  ...
  const limitEmail = await useStorage().getItem(`limit:email:${email}`)
  const limitIP = await useStorage().getItem(`limit:ip:${ip}`)
  ...
  await useStorage().setItem(`${salt}:${email}`, code, { ttl: 10 * 60 })
  await useStorage().setItem(`limit:email:${email}`, code, { ttl: 30 })
  await useStorage().setItem(`limit:ip:${ip}`, code, { ttl: 30 })
